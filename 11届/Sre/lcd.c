/*
  ����˵��: CT117EǶ��ʽ������LCD��������
  ��������: Keil uVision 4.10
  Ӳ������: CT117EǶ��ʽ������(�������ILI932Xϵ�С�uc8230Һ����������
  ��    ��: 2013-10-9
*/
#include "lcd.h"
#include "fonts.h"
 ...
 * Description:设置背景颜色。
 *输入:—颜色:指定背景色码RGB(5-6-5)。
 *输出:- BackColor:背景颜色的全局变量使用
 * LCD_DrawChar和LCD_DrawPicture函数。
 *返回值:None
 *******************************************************************************/
 ...
 *                       - Linex: x 0 . . 9
 * - Column:列的起始地址。
 *—Ascii:字符的Ascii码，必须在0x20到0x7E之间。
 *输出:无
 *返回值:None
 *******************************************************************************/
 ...
 * Return         : None
 *函数名称:lcd_writebp
 *描述:显示加载在内部Flash的位图图片。
 *输入:—BmpAddress: Bmp图片在Flash内部的地址。
 *输出:无
 *返回值:None
 *函数名称:lcd_writebp
 *描述:显示加载在内部Flash的位图图片。
 ...
 *描述:写入选定的LCD寄存器。
 *输入:- LCD_Reg:所选寄存器的地址。
 * - LCD_RegValue:写入所选寄存器的值。
 *输出:无
 *返回值:None
 *******************************************************************************/
void LCD_WriteReg(u8 LCD_Reg, u16 LCD_RegValue)
{
	uint32_t i = GPIOC->ODR;
	GPIOB->BRR = 0x0200;
	GPIOB->BRR = 0x0100;
	GPIOB->BSRR = 0x0020;
